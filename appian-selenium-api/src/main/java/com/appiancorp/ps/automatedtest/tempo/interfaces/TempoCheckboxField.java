...
public class TempoCheckboxField extends AbstractTempoField {
    ...
    private static final Logger LOG = LogManager.getLogger(TempoCheckboxField.class);
    private static final String XPATH_ABSOLUTE_CHECKBOX_FIELD_LABEL =
            Settings.getByConstant("xpathAbsoluteCheckboxFieldLabel");
    private static final String XPATH_ABSOLUTE_CHECKBOX_FIELD_INDEX =
            Settings.getByConstant("xpathAbsoluteCheckboxFieldIndex");
    private static final String XPATH_ABSOLUTE_CHECKBOX_FIELD_LABEL_INDEX =
            "(" + XPATH_ABSOLUTE_CHECKBOX_FIELD_LABEL + ")[%d]";
    private static final String XPATH_RELATIVE_CHECKBOX_FIELD_CHOICE_LABEL =
            Settings.getByConstant("xpathRelativeCheckboxFieldChoiceLabel");
    private static final String XPATH_RELATIVE_CHECKBOX_FIELD_CHOICE_INDEX =
            Settings.getByConstant("xpathRelativeCheckboxFieldChoiceIndex");
    private static final String XPATH_RELATIVE_CHECKBOX_FIELD_INPUT =
            Settings.getByConstant("xpathRelativeCheckboxFieldInput");
    private static final String XPATH_RELATIVE_CHECKBOX_FIELD_INPUT_SPAN =
            Settings.getByConstant("xpathRelativeCheckboxFieldInputSpan");
    ...
    public String getXpath(String... params) {
        ...
        if (isFieldIndex(fieldName)) {
            ...
            if (StringUtils.isBlank(name)) {
                return xpathFormat(
                        XPATH_ABSOLUTE_CHECKBOX_FIELD_INDEX + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT,
                        index);
            } else {
                return xpathFormat(XPATH_ABSOLUTE_CHECKBOX_FIELD_LABEL_INDEX +
                                TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT, name,
                        index);
            }

        } else {
            return xpathFormat(
                    XPATH_ABSOLUTE_CHECKBOX_FIELD_LABEL + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT,
                    fieldName);
        }
    }
    ...
    public void populate(WebElement fieldLayout, String... params) {
        ...
        WebElement checkboxField = fieldLayout.findElement(By.xpath(getXpathForCheckbox(false, fieldValue)));
        checkboxField.click();
    }
    ...
    public String capture(WebElement fieldLayout, String... params) {
        ...
        for (WebElement span : fieldLayout.findElements(By.xpath(XPATH_RELATIVE_CHECKBOX_FIELD_INPUT_SPAN))) {
            if (span.findElement(By.tagName("input")).isSelected()) {
                String value = span.findElement(By.tagName("label")).getText();
    ...
    public boolean contains(WebElement fieldLayout, String... params) {
        ...
        // For editable
        boolean checked;
        WebElement checkbox = fieldLayout.findElement(By.xpath(getXpathForCheckbox(true, params)));
        checked = checkbox.getAttribute("checked") != null;

        if (LOG.isDebugEnabled()) {
            LOG.debug("CONTAINS: Field value [" + fieldValue + "] is checked [" + checked + "]");
        }

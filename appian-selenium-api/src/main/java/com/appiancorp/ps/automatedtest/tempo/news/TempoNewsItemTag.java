...
public class TempoNewsItemTag extends TempoNewsItem implements Clickable, Refreshable {
    ...
    private static final Logger LOG = LogManager.getLogger(TempoNewsItemTag.class);
    ...
    public void waitFor(String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("WAIT FOR NEWS ITEM [" + newsText + "] and TAG [" + recordTag + "]");
        }
    ...
    public void click(String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("CLICK ON NEWS ITEM [" + newsText + "] and RECORD TAG [" + recordTag + "]");
        }

        try {
            WebElement tag = settings.getDriver()
                    .findElement(By.xpath(xpathFormat(XPATH_ABSOLUTE_NEWS_ITEM_RECORD_TAG, newsText, recordTag)));
            clickElement(tag);
        } catch (Exception e) {

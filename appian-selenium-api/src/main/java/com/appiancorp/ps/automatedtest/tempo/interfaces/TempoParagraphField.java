...
public final class TempoParagraphField extends AbstractTempoField implements Container {

    private static final Logger LOG = LogManager.getLogger(TempoParagraphField.class);
    private static final String XPATH_ABSOLUTE_PARAGRAPH_FIELD_LABEL =
            Settings.getByConstant("xpathAbsoluteParagraphFieldLabel");
    private static final String XPATH_ABSOLUTE_PARAGRAPH_FIELD_INDEX =
            Settings.getByConstant("xpathAbsoluteParagraphFieldIndex");
    private static final String XPATH_ABSOLUTE_PARAGRAPH_FIELD_LABEL_INDEX =
            "(" + XPATH_ABSOLUTE_PARAGRAPH_FIELD_LABEL + ")[%2$d]";
    private static final String XPATH_RELATIVE_PARAGRAPH_FIELD_INPUT =
            Settings.getByConstant("xpathRelativeParagraphFieldInput");
    ...
    public String getXpath(String... params) {
        ...
        if (isFieldIndex(fieldName)) {
            ...
            if (StringUtils.isBlank(name)) {
                return xpathFormat(
                        XPATH_ABSOLUTE_PARAGRAPH_FIELD_INDEX + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT,
                        index);
            } else {
                return xpathFormat(XPATH_ABSOLUTE_PARAGRAPH_FIELD_LABEL_INDEX +
                                TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT, name,
                        index);
            }
        } else {
            return xpathFormat(
                    XPATH_ABSOLUTE_PARAGRAPH_FIELD_LABEL + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT,
                    fieldName);
        }
    }
    ...
    public boolean contains(WebElement fieldLayout, String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("PARAGRAPH FIELD COMPARISON : Field value [" + fieldValue + "] compared to Entered value [" +
                    compareString + "]");
        }
    ...
    public void clear(WebElement fieldLayout, String... params) {
        ...
        textAreaField.sendKeys(
                Strings.repeat(Keys.BACK_SPACE.toString(), textAreaField.getAttribute("value").length()));
    }

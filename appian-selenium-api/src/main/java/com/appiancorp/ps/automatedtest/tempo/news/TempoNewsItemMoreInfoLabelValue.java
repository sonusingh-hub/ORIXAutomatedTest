...
public final class TempoNewsItemMoreInfoLabelValue extends TempoNewsItemMoreInfo implements WaitFor {

    private static final Logger LOG = LogManager.getLogger(TempoNewsItemMoreInfoLabelValue.class);
    ...
    public void waitFor(String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("WAIT FOR LABEL [" + label + "] and VALUE [" + value + "]");
        }
        ...
        try {
            (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(
                    ExpectedConditions.presenceOfElementLocated(By.xpath(xpathFormat(

...
public final class TempoImageField extends AbstractTempoField {

    private static final Logger LOG = LogManager.getLogger(TempoTextField.class);
    private static final String XPATH_ABSOLUTE_IMAGE_FIELD_LABEL =
            Settings.getByConstant("xpathAbsoluteImageFieldLabel");
    private static final String XPATH_ABSOLUTE_IMAGE_FIELD_INDEX =
            Settings.getByConstant("xpathAbsoluteImageFieldIndex");
    private static final String XPATH_ABSOLUTE_IMAGE_FIELD_LABEL_INDEX =
            "(" + XPATH_ABSOLUTE_IMAGE_FIELD_LABEL + ")[%2$d]";
    private static final String XPATH_RELATIVE_IMAGE_FIELD_INPUT =
            Settings.getByConstant("xpathRelativeImageFieldInput");
    ...
    public String getXpath(String... params) {
        ...
        if (isFieldIndex(fieldName)) {
            ...
            if (StringUtils.isBlank(name)) {
                return xpathFormat(
                        XPATH_ABSOLUTE_IMAGE_FIELD_INDEX + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT, index);
            } else {
                return xpathFormat(
                        XPATH_ABSOLUTE_IMAGE_FIELD_LABEL_INDEX + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT,
                        name,
                        index);
            }

        } else {
            return xpathFormat(XPATH_ABSOLUTE_IMAGE_FIELD_LABEL + TempoFieldFactory.XPATH_CONCAT_ANCESTOR_FIELD_LAYOUT,
                    fieldName);
        }
    }
    ...
    public String capture(WebElement fieldLayout, String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("IMAGE FIELD ALT : " + value);
        }
    ...
    public boolean contains(WebElement fieldLayout, String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("IMAGE FIELD COMPARISON : Field value [" + fieldValue + "] compared to Entered value [" +
                    compareString + "]");
        }

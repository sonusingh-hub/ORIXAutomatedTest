...
public class TempoChart extends AppianObject implements WaitForReturn {
    ...
    private static final Logger LOG = LogManager.getLogger(TempoChart.class);
    private static final String XPATH_ABSOLUTE_CHART_LABEL = Settings.getByConstant("xpathAbsoluteChartLabel");

    private static final String XPATH_RELATIVE_CHART_DOWNLOAD = Settings.getByConstant("xpathRelativeChartDownload");
    ...
    public void waitFor(String... params) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("WAIT FOR TEMPO CHART");
        }
    ...
    public boolean waitForReturn(boolean waitForPresent, int timeout, String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("WAIT FOR CHART [" + chartName + "]");
        }
    ...
    public void clickOnDownloadIcon(String chartLabel) {
        WebElement downloadChart =
                settings.getDriver().findElement(By.xpath(xpathFormat(XPATH_RELATIVE_CHART_DOWNLOAD, chartLabel)));
        (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds())))
                .until(ExpectedConditions.elementToBeClickable(downloadChart));

        clickElement(downloadChart);
    }

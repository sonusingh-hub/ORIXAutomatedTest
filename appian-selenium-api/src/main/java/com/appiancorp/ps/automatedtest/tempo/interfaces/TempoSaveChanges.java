...
public final class TempoSaveChanges extends AppianObject implements WaitFor, Clickable, Completeable {

    private static final Logger LOG = LogManager.getLogger(TempoSaveChanges.class);
    private static final String XPATH_ABSOLUTE_SAVE_CHANGES = Settings.getByConstant("xpathAbsoluteSaveChanges");
    private static final String XPATH_ABSOLUTE_FORM_SAVED_CONFIRMATION =
            Settings.getByConstant("xpathAbsoluteFormSavedConfirmation");
    ...
    public void waitFor(String... params) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("WAIT FOR SAVE CHANGES");
        }
    ...
    public boolean complete(String... params) {
        try {
            (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getNotPresentTimeoutSeconds()))).until(

...
public class TempoCardDirect extends TempoContainer implements WaitFor, WaitForReturn, Clickable {

    private static final Logger LOG = LogManager.getLogger(TempoCardDirect.class);
    ...
    public static TempoCardDirect getInstance(Settings settings) {
        return new TempoCardDirect(settings);
    }

    protected TempoCardDirect(Settings settings) {
        super(settings);
    }
    ...
    private void performClick(String... params) throws Exception {
        ...
        try {
            ...
            // If no clickable element found, try direct click on card
            try {
                clickElement(card);
                LOG.debug("Clicked directly on card element");
            } catch (Exception e2) {
    ...
    public void click(String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("CLICK DIRECT ON CARD [" + cardName + "]");
        }
        ...
        try {
            ...
            try {
                ...
                throw ExceptionBuilder.build(e2, settings, "Card Direct", cardName);
            }
        } catch (Exception e) {
            throw ExceptionBuilder.build(e, settings, "Card Direct", cardName);
        }
    }
    ...
    public boolean waitForReturn(boolean waitForPresent, int timeout, String... params) {
        ...
        try {
            ...
            throw ExceptionBuilder.build(e, settings, "Wait for Card Direct", cardName);
        }
    }

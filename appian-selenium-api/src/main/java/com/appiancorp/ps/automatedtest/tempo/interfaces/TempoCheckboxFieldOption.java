...
public final class TempoCheckboxFieldOption extends TempoCheckboxField implements Clickable {

    private static final Logger LOG = LogManager.getLogger(TempoCheckboxFieldOption.class);
    private static final String XPATH_ABSOLUTE_CHECKBOX_FIELD_OPTION =
            Settings.getByConstant("xpathAbsoluteCheckboxFieldOption");
    private static final String XPATH_ABSOLUTE_CHECKBOX_FIELD_OPTION_INDEX =
            "(" + XPATH_ABSOLUTE_CHECKBOX_FIELD_OPTION + ")[%2$d]";
    ...
    public WebElement getOptionLayout(String optionName) {
        if (isFieldIndex(optionName)) {
            ...
            return settings.getDriver()
                    .findElement(By.xpath(xpathFormat(XPATH_ABSOLUTE_CHECKBOX_FIELD_OPTION_INDEX, oName, index)));
        } else {
            return settings.getDriver()
                    .findElement(By.xpath(xpathFormat(XPATH_ABSOLUTE_CHECKBOX_FIELD_OPTION, optionName)));
        }
    }
    ...
    public void waitFor(String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("CHECKBOX OPTION WAIT FOR [" + optionName + "]");
        }

...
public class TempoSectionValidation extends TempoSection implements WaitForMultiple, Captureable {
    ...
    private static final Logger LOG = LogManager.getLogger(TempoSectionValidation.class);

    protected static final String XPATH_ABSOLUTE_SECTION_FIELD_LAYOUT =
            Settings.getByConstant("xpathAbsoluteSectionFieldLayout");
    protected static final String XPATH_ABSOLUTE_SECTION_FIELD_LAYOUT_INDEX =
            Settings.getByConstant("xpathAbsoluteSectionFieldLayoutIndex");
    protected static final String XPATH_RELATIVE_SECTION_VALIDATION_MESSAGE_SPECIFIC_VALUE =
            Settings.getByConstant("xpathRelativeSectionValidationMessageSpecificValue");
    protected static final String XPATH_RELATIVE_SECTION_VALIDATION_MESSAGE =
            Settings.getByConstant("xpathRelativeSectionValidationMessage");
            ...
            for (String validationMessage : validationMessages) {
                (new WebDriverWait(settings.getDriver(), Duration.ofSeconds(settings.getTimeoutSeconds()))).until(
                        ExpectedConditions.presenceOfElementLocated(By.xpath("(" +
                                xpathLocator + ")" +
                                xpathFormat(XPATH_RELATIVE_SECTION_VALIDATION_MESSAGE_SPECIFIC_VALUE,
                                        validationMessage))));
            }
        } catch (Exception e) {
            throw ExceptionBuilder.build(e, settings, "Wait for Section Field", sectionName,
    ...
    public String capture(String... params) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("GET SECTION VALIDATIONS [" + sectionName + "]");
        }
